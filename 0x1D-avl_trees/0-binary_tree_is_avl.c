...
int binary_tree_is_avl(const binary_tree_t *tree)
{
	...
	if (tree == NULL)
		return (0);
	check = bst_recursive(tree, INT_MIN, INT_MAX);
	if (check == 0)
		return (0);
	check = balanced_preorder(tree);
	if (check == 0)
		return (0);
	return (1);
}
...
/**
 * bst_recursive - recursively checks if tree is valid binary search tree
 * @tree: pointer to the root node of the tree to check
...
int bst_recursive(const binary_tree_t *tree, int min, int max)
{
	...
	if (local_min == min && local_max == max)
		return (1);
	left = bst_recursive(tree->left, min, tree->n);
	right = bst_recursive(tree->right, tree->n, max);
	return (left * right);
}

/**
 * balanced_preorder - determines if the tree is balanced at all levels
...
int balanced_preorder(const binary_tree_t *tree)
{
	...
	if (balanced  == 0)
		return (0);
	balanced = balanced_preorder(tree->left);
	if (balanced == 0)
		return (0);
	balanced = balanced_preorder(tree->right);
	if (balanced == 0)
		return (0);
	return (1);
}
...
/**
 * balanced - determines if the tree is balanced
 * balanced = difference between left and right subtrees cannot be more than 1
 * @tree: pointer to the root node of the tree to check
 *
...
/**
 * height - determines the height of the subtree
